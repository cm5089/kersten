// Standalone Kersten Authority Audit Widget — mount via #kersten-authority-audit-widget-root

(function() {
    const loadKerstenAuthorityAuditWidget = (container) => {
        ...
        container.id = 'kersten-authority-audit-widget';
        container.className = 'pesticide-load-widget kersten-authority-audit-widget';
        container.setAttribute('data-kersten-authority', 'audit');
        container.style.cssText = "background:#f9fbf9; border:2px solid #27ae60; border-radius:12px; margin:30px auto; max-width:1200px; width:100%; box-sizing:border-box; font-family:sans-serif; overflow:hidden; box-shadow:0 4px 15px rgba(39,174,96,0.1); clear:both; display:block;";
        ...
        container.innerHTML = `
            ...
            <div style="padding:25px;">
                ...
                <div style="background:#f9fffb; padding:20px; border-radius:10px; border:1px dashed #27ae60; margin-bottom:20px;">
                    ...
                    <select data-audit-method style="width:100%; padding:10px; border:1px solid #ccc; border-radius:6px; font-size:14px; margin-bottom:15px; background:white;">
                        ${isSweeperContext ?
                    ...
                    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
                        <div>
                            <label data-audit-lbl-1 style="display:block; font-size:12px; font-weight:bold; color:#004d26; margin-bottom:5px;">${initialInputLabel}</label>
                            <input type="number" data-audit-input1 placeholder="${placeholder1}" style="width:100%; padding:12px; border:1px solid #ccc; border-radius:6px; font-size:15px;">
                        </div>
                        <div>
                            <label style="display:block; font-size:12px; font-weight:bold; color:#004d26; margin-bottom:5px;">${distLabel}</label>
                            <input type="number" data-audit-input2 placeholder="${placeholder2}" style="width:100%; padding:12px; border:1px solid #ccc; border-radius:6px; font-size:15px;">
                        </div>
                    </div>
                    <button data-audit-btn-exec style="width:100%; margin-top:20px; background:#004d26; color:white; border:none; padding:15px; border-radius:8px; font-weight:bold; cursor:pointer; font-size:14px; letter-spacing:1px; text-transform:uppercase;">
                        Calculate Savings (Planet & Wallet)
                    </button>
                </div>
                <div data-audit-result-display style="margin-top:20px; display:none; border-top:2px solid #eee; padding-top:20px; animation: fadeIn 0.5s;"></div>
            </div>`;

        const methodSelect = container.querySelector('[data-audit-method]');
        const inputLabel1 = container.querySelector('[data-audit-lbl-1]');
        const inputField1 = container.querySelector('[data-audit-input1]');
        const execBtn = container.querySelector('[data-audit-btn-exec]');
        const resultDisplay = container.querySelector('[data-audit-result-display]');
        ...
        if (execBtn) {
            execBtn.onclick = function() {
                const val1 = parseFloat(container.querySelector('[data-audit-input1]').value) || 0;
    ...
    const mountWidget = () => {
        const root = document.getElementById('kersten-authority-audit-widget-root');
        if (root && !root.dataset.initialized) {
            root.dataset.initialized = 'true';
            loadKerstenAuthorityAuditWidget(root);
        }
    };
