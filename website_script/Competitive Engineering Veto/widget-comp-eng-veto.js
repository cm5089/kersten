// Standalone Competitive Engineering Veto — mount via #kersten-veto-table-root
// Optional: set data-veto-context="kubota|hatz|electric" on the root element
...
(function() {
    ...
    const determineContext = (root) => {
        const forced = (root.dataset.vetoContext || '').toLowerCase();
        if (forced && comparisonLogic[forced]) return forced;
        ...
        const isThermalProduct = (pageText.includes("eco weedkiller") || pageText.includes("hot water") || pageText.includes("steam"))
            && (pageText.includes("specification") || pageText.includes("technical"));
        if (isThermalProduct) return "hatz";

        return null;
    };

    const loadCompEngVeto = (container) => {
        const context = determineContext(container);
        if (!context) {
        ...
        container.id = 'kersten-veto-table';
        container.className = 'kersten-comp-eng-veto';
        container.style.cssText = "margin: 60px auto 40px auto; max-width: 1200px; border: 2px solid #004d26; border-radius: 8px; overflow: hidden; background: white; font-family: sans-serif; clear: both; box-shadow: 0 5px 15px rgba(0,0,0,0.08); display: block; box-sizing: border-box; width: 100%;";
    ...
    const mountWidget = () => {
        const root = document.getElementById('kersten-veto-table-root');
        if (root && !root.dataset.initialized) {
            root.dataset.initialized = 'true';
            loadCompEngVeto(root);
        }
    };
